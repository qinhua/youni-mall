...
(function (global, factory) {
  ...
  var myMixin = (_myMixin2 = {
     ...
     */
    loadData: function loadData(url, params, method, dataType, async, beforeCb, successCb, errCb) {
      $.ajax({
        type: method || 'GET',
        url: url,
        async: async || true,
     ...
     * @param pageSize 分页大小
     */
    loadMore: function loadMore(ele, listEle, cb, pageSize) {
     ...
     * @param callback
     */
    shake: function shake(thresholds, direction, callback) {
     ...
     */
    hover: function hover(targetId, hoverCls) {
    ...
    attachClick: function attachClick() {
    ...
    checkStr: function checkStr(str) {
      if (escape(str).indexOf("%u") != -1) alert("不能含有汉字");else if (str.match(/\D/) != null) {
        alert('不能含有字母');
      }
    },
     ...
     * @param end 终点
     */
    mixStr: function mixStr(str, _mixStr, start, end) {
      _mixStr = _mixStr || "***";
      return str.replace(str.slice(start, end), _mixStr);
    },
    /**
     * 事件统计
     * @param adsName
     ...
     * @param eventName
     */
    statistic: function statistic(adsName, pageName, eventName) {
      setTimeout(function () {
     ...
     * @param param 分享配置obj
     */
    setShare: function setShare(param) {
    ...
    param: function param(obj) {
      var retData = [];
      for (var e in obj) {
        if (_typeof(obj[e]) === 'object') for (var i = 0; i < obj[e].length; i++) {
    ...
    // 不刷新改变url地址
    setUrl: function setUrl(params, title, page) {
      var currentState = history.state;
      window.history.pushState(params, title, page);
    },
    // 重置url地址
    resetUrl: function resetUrl(title, page) {
      var currentState = history.state;
     ...
     * @param url
     */
    jumpTo: function jumpTo(url, time, params) {
    ...
    // 返回某一页
    pageBack: function pageBack(pageNum, isRefresh) {
      if (history.length > 1) {
        isRefresh ? self.location = document.referrer : null;
        pageNum ? history.go(pageNum) : history.go(-1);
      }
    },
    openWin: function openWin(url, title, h, w, t, l, tool, menu, scro, resize, loc, sta) {
    ...
    lightPop: function lightPop(msg, autoHide) {
    ...
    getInfo: {
      ...
      storeBp: function storeBp() {
        var token = "bps" + myMixin.config.type;
        if (!localStorage.getItem(token) || localStorage.getItem(token) === "{}") {
          localStorage.setItem(token, _JSON$stringify(myMixin.getURLParams()));
        }
      },
      baseParas: function baseParas(obj) {
        var bp = {};
        var token = "bps" + myMixin.config.type;
    ...
    //ajax请求
    ajaxs: function ajaxs(type, url, data, callback01, callback02, callback03, callback04) {
      $.ajax({
        type: type, //请求方式
        url: url, //请求的url地址
    ...
    deferrsed: function deferrsed(type, url, callback) {
    ...
    //判断手机横竖屏状态：
    checkOrientation: function checkOrientation(callback01, callback02) {
     ...
     * @param ele
     */
    verCodeBtn: function verCodeBtn(time, ele) {
      var timer = null;
     ...
     * 显示/关闭弹窗
     */
    showPop: function showPop(msg, cls, popHeight, cb, closeCb) {
     ...
     * @constructor
     */
    AlertPop: function AlertPop(pop, mask) {
    ...
    //滑动到某处
    scroll: function scroll(num, time) {
     ...
     * @param trigger,触发元素
     */
    back2Top: function back2Top(trigger) {
    ...
    //swiper通用配置
    swiper: {
      common: function common(targetCls, direction) {
      ...
      marquee: function marquee(targetCls, direction) {
        if (typeof Swiper != 'undefined') {
          return new Swiper(targetCls || '.swiper-container', {
            // Optional parameters
            initialSlide: 0,
    ...
    Arrs: {
       ...
       */
      //01.简单数组正序排列
      sortNumAray: function sortNumAray(a, b) {
        if (a < b) {
          return -1; // a排在b的前面
        } else if (a > b) {
          return 1; // a排在b的后面
      ...
      // 生成从1到指定值的数组
      generateArr: function generateArr(num) {
        var tmp = [];
        return function () {
          tmp.unshift(num);
          num--;
      ...
      // 05.拷贝数组
      copyArr: function copyArr(arr, start, end) {
        if (start) {
    ...
    // 拷贝对象
    copyObj: function copyObj(obj, filter) {
      var b = {};
      for (var i in a) {
        if (filter) {
    ...
    // 格式化json
    formateJson: function formateJson(json) {
      var i = 0,
          il = 0,
          tab = "    ",
          newJson = "",
    ...
    Rdn: {
      //01.生成从1到任意值的数字
      rdnTo: function rdnTo(end) {
        return parseInt(Math.random() * end + 1);
      },
      //02.生成从任意值到任意值的数字
      rdnBetween: function rdnBetween(start, end) {
        return Math.floor(Math.random() * (end - start) + start);
      },
      //03.从数组中随机取出一些数（arr：原数组，len：需要取的个数）
      rdnFromArr: function rdnFromArr(arr, len) {
        var tmpArr = [];
        if (len > 1) {
      ...
      //04.生成从任意值开始的指定个数字
      rdnCustom: function rdnCustom(start, lens) {
        var arr = [];
        //给原始数组rdnArr赋值
       ...
       * @returns {string}
       */
      rdnCountUp: function rdnCountUp(baseNum, ratio, limit) {
        var curNum,
            lastNum = parseInt(myMixin.locals.get("meLstAniNum") || baseNum);
       ...
       * @returns {number}
       */
      getFakeCount: function getFakeCount(startTime, threshold) {
       ...
       * @returns {number}
       */
      genSignUpNum: function genSignUpNum(timeInterval, storageName) {
        ...
        if (localTime) {
          ...
          if (oldMin < 0 || newMin < 0) {
            localStorage.setItem(storageName, _JSON$stringify(tempTimeAray));
          }
    ...
    isNumber: function isNumber(nums) {
      return !isNaN(Number(nums));
    },
    isLeapYear: function isLeapYear(year) {
      return new Date(year).isLeapYear();
    },
    formatTime: function formatTime(timeStr, fmt) {
    ...
    locals: {
      ...
      //检查是否有指定字段
      check: function check(name, callback01, callback02) {
     ...
     */
    cookies: {
      set: function set(name, value, expireHours) {
        var cookieString;
        if ((typeof value === "undefined" ? "undefined" : _typeof(value)) === "object") {
          cookieString = name + "=" + _JSON$stringify(value);
        } else {
     ...
     */
    weixinAuth: function weixinAuth(success) {
      var USERINFO = {};
      var Storage = {
        ...
        set: function set(val) {
          window.localStorage.setItem(Storage.name, _JSON$stringify(val));
        }
      };
    ...
    // 计算时间消耗
    timeCost: function timeCost(cb, alias) {
      console.time(alias);
      cb ? cb() : null;
    ...
    formatDate: function formatDate(now, isHans, level) {
      level = level || 3;
     ...
     * @returns {boolean}
     */
    compareCurrentDate: function compareCurrentDate(d1) {
     ...
     * @returns {boolean}
     */
    keepFresh: function keepFresh(endTime, cb, interval) {
      var timer = null;
     ...
     */
    //单个倒计时
    getRCount: function getRCount(target, box) {
    ...
    //传入毫秒数时
    getMsCount: function getMsCount(msd) {
      //console.log(arguments);//最好直接传入数字类型
      var t, d, h, m, s;
      var $days = $(".days");
      var $hours = $(".hours");
     ...
     */
    getMRCount: function getMRCount(target, obj, isMillis, cb) {
      var t, d, h, m, s;
     ...
     * @returns {*}
     */
    niceTime: function niceTime(timeStamp) {
     ...
     * @param time
     */
    counter: function counter(obj, time) {
      var initTime = time;
    ...
    // 禁止滑动
    cancleScroll: function cancleScroll(obj) {
     ...
     */
    //获取样式值
    getStyle: function getStyle(obj, styleName) {
     ...
     * @returns {*}
     */
    getObjLen: function getObjLen(o) {
    ...
    //过滤对象中自带属性
    exceptOwnProperty: function exceptOwnProperty(obj) {
     ...
     * @returns {{}}
     */
    xmlToJson: function (_xmlToJson) {
      function xmlToJson(_x) {
        return _xmlToJson.apply(this, arguments);
      }

      xmlToJson.toString = function () {
        return _xmlToJson.toString();
      };
     ...
     * @param time
     */
    timeOut: function timeOut(func, time) {
      setTimeout(func, time);
    },

    //------ 字符串操作
    //去除字符串空格
    trimStr: function trimStr(str, is_global) {
      var result;
    ...
    //字符串中指定位置插入字符
    insertStr: function insertStr(str, flg, sn) {
      var newstr = "";
    ...
    delStr: function delStr(str, x, num) {
     ...
     * @returns {string|*}
     */
    ellipsis: function ellipsis(str, len, start, end) {
      start = start || 3;
     ...
     */
    prefixNum: function prefixNum(num, pre, n) {
     ...
     *@param num
     */
    numToThousand: function numToThousand(num, fixNum) {
      if ((num + "").trim() == "") {
        return "";
      }
      if (isNaN(num)) {
     ...
     *@param str
     */
    thousandToNum: function thousandToNum(str) {
      var num = $.trim(str);
      var ss = num.toString();
      if (ss.length == 0) {
        return "0";
     ...
     * @returns {string}
     */
    numToChinese: function numToChinese(num) {
     ...
     */
    mHover: function mHover(obj, classFocus) {
    ...
    isOnlineEvt: function isOnlineEvt() {
    ...
    isArray: function isArray(sources) {
      return '[object Array]' == Object.prototype.toString.call(sources);
    },

    isDate: function isDate(sources) {
      return {}.toString.call(sources) === "[object Date]" && sources.toString() !== 'Invalid Date' && !isNaN(sources);
    },

    isElement: function isElement(sources) {
      return !!(sources && sources.nodeName && sources.nodeType == 1);
    },

    isFunction: function isFunction(sources) {
      return '[object Function]' == Object.prototype.toString.call(sources);
    }

  }, _defineProperty(_myMixin2, "isNumber", function isNumber(sources) {
    return '[object Number]' == Object.prototype.toString.call(sources) && isFinite(sources);
  }), _defineProperty(_myMixin2, "isObject", function isObject(sources) {
    return '[object Object]' == Object.prototype.toString.call(sources);
  }), _defineProperty(_myMixin2, "isString", function isString(sources) {
    return '[object String]' == Object.prototype.toString.call(sources);
  }), _defineProperty(_myMixin2, "isBoolean", function isBoolean(sources) {
    return typeof sources === 'boolean';
  }), _defineProperty(_myMixin2, "playAudio", function playAudio(audioSrc, idName, toggleEle, autoPlay, loop, isCheckStatus) {
  ...
  }), _defineProperty(_myMixin2, "switchAudio", function switchAudio(ele, audioEle) {
  ...
  }), _defineProperty(_myMixin2, "toggleSound", function toggleSound(audio, toggleEle) {
  ...
  }), _defineProperty(_myMixin2, "swSound", function swSound(audio, isClose, isMute, cb) {
    try {
  ...
  }), _defineProperty(_myMixin2, "iScrollClick", function iScrollClick() {
  ...
  }), _defineProperty(_myMixin2, "storeUser", function storeUser(key, obj) {
    myMixin.locals.set(key, obj);
  }), _defineProperty(_myMixin2, "readUser", function readUser(key, ele1, ele2) {
  ...
  }), _defineProperty(_myMixin2, "hasTouch", function hasTouch() {
    var touchObj = {};
    touchObj.isSupportTouch = "ontouchend" in document ? true : false;
    touchObj.isEvent = touchObj.isSupportTouch ? "touchstart" : "click";
    return touchObj.isEvent;
  }), _defineProperty(_myMixin2, "switchScroll", function switchScroll(status) {
    status === -1 ? $("html,body").removeClass("disable-scroll") : $("html,body").addClass("disable-scroll");
  }), _defineProperty(_myMixin2, "theRaf", {
    activeRaf: function activeRaf() {
      var lastTime = 0;
  ...
  }), _defineProperty(_myMixin2, "imgError", function imgError(picEle, defaultSrc, callback) {
  ...
  }), _defineProperty(_myMixin2, "webUploader", function webUploader() {
...
//# sourceMappingURL=myMixin-compiled.js.map