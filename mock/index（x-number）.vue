  ...
  <div class="weui-cell">
    ...
    <div class="weui-cell__ft vux-cell-primary" :class="{'vux-number-round': buttonStyle === 'round'}" v-show="!readonly" style="font-size:0">
      <div :style="{float:align}">
        <a @click.stop="sub" class="vux-number-selector vux-number-selector-sub" :class="{'vux-number-disabled':disabledMin}">
        ...
        <input v-model.number="currentValue" :name="name" class="vux-number-input" :style="{width: width}" :readonly="!fillable" pattern="[0-9]*" type="input" @blur="blur" />
        <a @click.stop="add" class="vux-number-selector vux-number-selector-plus" :class="{'vux-number-disabled':disabledMax}">
...
export default {
  ...
  watch: {
    currentValue (newValue, old) {
      ...
      this.$emit('on-change', {type: this.type, value: this.currentValue, event: this.event})
    },
    value (newValue) {
