    ...
    mounted() {
      ...
      if (!me.locals.get('ynWxUser')) {
        if (me.isWeixin) {
          ...
          if (urlParam.code) {
            alert('1')
            $.getJSON('https://api.weixin.qq.com/sns/oauth2/access_token?appid=' + vm.appId + '&secret=' + vm.appSecret + '&code=' + urlParam.code + '&grant_type=authorization_code',function(res){
