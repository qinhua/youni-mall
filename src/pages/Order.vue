...
<script>
  ...
  export default {
    ...
    components: {Tab, TabItem, Group, XInput},
    beforeMount() {
      me = window.me
    },
    mounted() {
    ...
    methods: {
      ...
      payDeposite(id) {
        ...
        if (vm.isPosting) return false
        vm.isPosting = true
        vm.confirm('请填写桶数？', '<div class="depositeModal"><input type="number" placeholder="输入数量（桶）" required v-model.number="bucketAmount"></div>', function () {
          vm.loadData(depositApi.add, {
            sellerId: id,
            bucketNum: vm.bucketAmount
          }, 'POST', function (res) {
            vm.isPosting = false
            var resD = res.data
            vm.pay(resD, true)
